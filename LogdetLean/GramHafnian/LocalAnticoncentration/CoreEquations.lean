...
/-!
# Paper facing endpoints for the current manuscript
...
/-- manuscript Eq. (2): the observable is the hafnian of the transpose Gram matrix.
...
/-- manuscript Eq. (3): the standard deviation is the literal second absolute
...
/-- manuscript Eq. (3), displayed finite product form.  `oddPairingNat n` is the
...
/-- manuscript Eq. (4): uniformly shifted quadratic small ball theorem, including
the probability cap. -/
...
/-- manuscript Eq. (5): exact product definition of the finite coefficient. -/
...
/-- manuscript Eq. (6): the three cofactor quantities are literal definitions.
...
/-- manuscript Eq. (7), algebraic part: expansion in the exposed final column. -/
...
/-- manuscript Eq. (7), law part: conditionally on fixed past columns, the exposed
...
/-- manuscript Eq. (8): the exact shifted disk estimate before normalization. -/
...
/-- manuscript Eq. (9): Fourier cofactor inverse moment comparison. -/
theorem eq9_fourier
...
/-- manuscript Eq. (10): conditional Wishart inverse moment comparison. -/
...
/-- manuscript Eq. (11): the iterated inverse variance product, in the exact
...
/-- The first level used in manuscript Eq. (11) is exactly `(k-1)^{-1}`. -/
