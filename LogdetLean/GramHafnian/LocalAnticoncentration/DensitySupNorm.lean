...
/-!
# Literal supremum norm endpoint for the current manuscript density

This module packages the pointwise density estimate as the exact uniform norm
statement printed after Equation (13) in the manuscript.
-/
...
def localAnticoncentrationGramHafnianDensitySupNorm
    {r k : ℕ} (hr : 1 ≤ r) : ℝ :=
...
theorem localAnticoncentrationGramHafnianDensitySupNorm_eq_at_zero
    {r k : ℕ} (hr : 1 ≤ r) (hk : 4 * r ≤ k) :
    localAnticoncentrationGramHafnianDensitySupNorm (k := k) hr =
  ...
  have hnonneg : ∀ w : ℂ, 0 ≤ f w := fun w ↦
    localAnticoncentrationGramHafnianDensity_nonneg hr w
  have hmax : ∀ w : ℂ, f w ≤ f 0 := fun w ↦
    localAnticoncentrationGramHafnianDensity_le_at_zero hr hk w
  have hbounded : BddAbove (Set.range fun w : ℂ ↦ ‖f w‖) := by
    refine ⟨f 0, ?_⟩
    rintro _ ⟨w, rfl⟩
    simpa [Real.norm_eq_abs, abs_of_nonneg (hnonneg w)] using hmax w
  apply le_antisymm
  · unfold localAnticoncentrationGramHafnianDensitySupNorm
...
`π σ² ‖f‖∞ ≤ B`. -/
theorem pi_sigma_sq_mul_localAnticoncentrationGramHafnianDensitySupNorm_le
    {r k : ℕ} (hr : 1 ≤ r) (hk : 4 * r ≤ k) :
    Real.pi * gramHafnianSigma k r ^ 2 *
        localAnticoncentrationGramHafnianDensitySupNorm (k := k) hr ≤
      shiftedAnticoncentrationConstant k r := by
  rw [localAnticoncentrationGramHafnianDensitySupNorm_eq_at_zero hr hk]
  exact pi_sigma_sq_mul_localAnticoncentrationGramHafnianDensity_le hr hk 0
